...
- **Squash→encoder semaphore handoff** (`compose_via_squasher` `vkQueueWaitIdle` → GPU→GPU) — full plan in [`SEMAPHORE_HANDOFF_SCOPE.md`](SEMAPHORE_HANDOFF_SCOPE.md) + [`B2_2B_PLAN.md`](B2_2B_PLAN.md). **All compile-checkable + CI-checkable parts are landed and green**; only the runtime behaviour-flip is left, RTX-host-gated. Status:
  - *Slice A* (forward timeline semaphore created/signalled in `comp_alvr`, handle+value over `alvr_oxr_submit_layers`, **ABI v9**) — ✅ landed, CI green.
  - *B1* (encoder: CUDA stream + `cuWaitExternalSemaphoresAsync` on the forward semaphore before the copy; encode still synchronous) — ✅ landed.
  - *B2.2a* (reverse "consumed" semaphore + CUDA handle-type reporting, **ABI v10**; encoder imports both type-correctly and signals the reverse after the copy) — ✅ landed. **All three are additive / behaviour-preserving** — `comp_alvr` still CPU-waits and the reverse signal is inert, so **no measured win yet**.
  - *B2.2b* (the flip: drop the CPU waits + cross-queue FFR wait + move encode off the compositor thread) — **not started, RTX-host-gated.** `B2_2B_PLAN.md` splits it into *partial* (move only `EncodeFrame` to a worker via a staging buffer — no reuse hazard, reverse semaphore unused) and *full* (compositor returns right after `vkQueueSubmit`; needs the per-ring-slot reverse-wait state machine + drop-consistency). Bring up **partial first** on TESTHOST and measure `oxr_pacing` before paying for full.
