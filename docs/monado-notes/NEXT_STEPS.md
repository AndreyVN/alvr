...
* Quad / cylinder / equirect / cube / passthrough layer support — rasterise into a single projection image before encoding.
  ...
  * ⏸ Slice 2: actual Vulkan rasterisation. **Scope nailed down 2026-05-23 in [`PHASE7_SLICE2_SCOPE.md`](PHASE7_SLICE2_SCOPE.md)**. Headline: don't roll a custom quad shader — reuse Monado's existing layer squasher (`comp_high_level_render.h` + `comp_high_level_scratch.h`), which already handles every XRT layer type and exposes its output as an `xrt_image_native` handle that the bridge already consumes. Sub-slice ladder: 2a scope doc (done), 2b scratch lifecycle in compositor init/destroy, 2c squasher dispatch in layer_commit, 2d hardware-gated verification with an overlay-using OpenXR client, 2e (stretch) GFX → CS dispatch swap.
