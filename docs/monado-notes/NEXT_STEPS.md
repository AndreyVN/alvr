...
- **Optional / perf** — Slice 2e (GFX→CS squasher dispatch swap, only if measured); the `compose_via_squasher` `vkQueueWaitIdle` → semaphore handoff is **half-done**: *Slice A* (exported timeline semaphore created in `comp_alvr`, signalled from the squash + FFR submits, handle+value forwarded through `alvr_oxr_submit_layers`, bridge **ABI v9**) is landed and compile-verified (AMD host) but **additive only** — the CPU `vkQueueWaitIdle`/FFR fence-wait still stay and the encoder ignores the new fields, so there is **no measured win yet**; *Slice B* (drop the CPU waits + add `cuWaitExternalSemaphoresAsync` in `VkEncoderBackend` + move encode off the compositor thread) is the actual latency win and is **RTX-host-gated** for any before/after `oxr_pacing` measurement. Gate H (per-eye foveation behaviour) on an eye-tracked headset.
