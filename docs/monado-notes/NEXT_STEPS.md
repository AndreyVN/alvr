...
- **Small ergonomics** — make `build-openxr-runtime` auto-build the `alvr_server_openxr` cdylib so a stale DLL can't ship silently (see lessons); optionally surface `ALVR_MONADO_SOURCE_DIR` as a setting; propagate encoder-unavailable out of `alvr_oxr_submit_layers` as non-OK; populate `comp_scratch_single_images::native_images[idx].size` for the FFR-off path.
...
- **Stale cdylib trap:** `build-openxr-runtime` deploys `target/{debug,release}/alvr_server_openxr.dll` without rebuilding the Rust crate — a stale DLL once masqueraded as a baseline-present bug across a whole bisect. Rebuild the cdylib explicitly until the auto-build follow-up lands.
