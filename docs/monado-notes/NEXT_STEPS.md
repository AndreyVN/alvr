...
* Quad / cylinder / equirect / cube / passthrough layer support — rasterise into a single projection image before encoding.
  ...
  * ⏸ Slice 2c.1 — distortion target wiring so `compose_via_squasher` can actually dispatch. Two paths: (a) allocate a dummy 1x1 distortion target and pass it via `chl_frame_state_gfx_set_target`; (b) add a squash-only dispatch entry point on the fork side. The dispatch wrapper in `comp_alvr.c::compose_via_squasher` is preserved inside `#if 0` so the rest of the integration is a single Boolean flip plus the target plumbing.
