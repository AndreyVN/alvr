...
- ✅ **CI: Windows OpenXR-runtime job LANDED 2026-05-22**. `.github/workflows/openxr.yml` already built `XRT_BUILD_DRIVER_ALVR=ON XRT_FEATURE_COMP_ALVR=ON` (`cargo xtask build-openxr-runtime --enable-alvr-driver`) and ran Monado's ctest (~3759 assertions). This session added a `Run OpenXR-mode unit tests` step that runs the two test suites with no other CI coverage: `cargo test -p alvr_launcher` (8 session.json runtime-mode round-trip tests in `alvr/launcher/src/actions.rs`) and `cargo test -p alvr_server_core metrics_exporter` (8 `OxrPacing`/`OxrLayerTypes`/`Battery` aggregator tests in `alvr/server_core/src/metrics_exporter.rs`). Both green locally — `8 passed; 0 failed` each. PR path-gate also expanded to include `alvr/launcher/**` and `alvr/server_core/src/metrics_exporter.rs` so changes to the tested surfaces trigger the workflow on PR. **Verification ceiling**: the new test steps haven't been observed running on Actions yet (next push to `openxr` will be the first); both pass locally on the same Windows toolchain CI uses.
  * ⚠️ **BLOCKED — `openxr.yml` is red on every run (maintainer action required, not a code bug).** Confirmed 2026-05-25 via the GitHub REST API: every run, including commits *before* any recent work (`140d6934`, `dc021485`, `7427afac`), fails at step 2 `actions/checkout@v6` and skips every later step. Cause: the `openxr/` submodule points at **private** repo `AndreyVN/monado` (404 unauthenticated), and the default `GITHUB_TOKEN` cannot read a *different* private repo. The checkout already passes `token: ${{ secrets.MONADO_PAT }}` (wired in `1efed4c9`), so the only missing piece is the secret itself.
    ...
    * **Token rotation:** when the PAT expires the checkout will 404 again with the same symptom — repeat steps 1–2 with a fresh token. There is no in-repo way to detect impending expiry; treat a sudden checkout-only failure as the signal.
