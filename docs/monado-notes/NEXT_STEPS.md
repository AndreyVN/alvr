...
- **FFR-off + H.264 silently streams no video** (found 2026-06-03). With foveated encoding disabled the combined frame is full-res (e.g. 4288 wide), which exceeds H.264 NVENC's 4096 max width → `nvEncInitializeEncoder` returns error 8 and the streamer logs `OpenXR mode: NVENC encoder unavailable; no video will stream`. FFR-on works only because the foveated width is <4096. Robustness fix worth doing: when the encoder frame exceeds the codec's max dimension, auto-select HEVC (max 8192) or clamp/scale, instead of failing. (HEVC at 4288 works fine on RTX 3090 + Quest 3.)
- **Controllers** — ✅ implemented & headset-verified (see shipped ledger). Two minor refinements remain, both optional: (a) grip and aim currently resolve to the **same** streamed pose — add a distinct aim offset if an app needs a precise pointer ray; (b) **usage note, not a bug:** controllers only track when Quest **hand tracking is OFF** (hand-tracking mode makes the Quest report controllers as `tracking: NONE`).
...
- **Squash→encoder semaphore handoff** (`compose_via_squasher` `vkQueueWaitIdle` → GPU→GPU) — **effectively complete & headset-verified on RTX 3090.** Plan/measurements in [`SEMAPHORE_HANDOFF_SCOPE.md`](SEMAPHORE_HANDOFF_SCOPE.md) + [`B2_2B_PLAN.md`](B2_2B_PLAN.md). Status:
  - *Slice A* (forward timeline semaphore, **ABI v9**), *B1* (encoder CUDA stream + forward wait), *B2.2a* (reverse "consumed" semaphore + handle-type, **ABI v10**) — ✅ landed, CI green, additive. Handle type = OPAQUE_WIN32/`TIMELINE_WIN32`; both imports confirmed OK on hardware.
  - *B2.2b-partial* (move `EncodeFrame` to a worker thread via a 3-slot staging pool; comp_alvr unchanged) — ✅ **landed + headset-verified** (clean video, no drops). **Measured win: ~2.66 ms/frame off the compositor thread** (bridge call ~3.4 ms → 0.74 ms @ 2560×1184).
  - *B2.2b-full* (drop the CPU waits + reverse-wait state machine) — **DECLINED (measured).** comp_alvr `cpu_us=492` µs, so full removes only ~0.49 ms/frame — not worth its reverse-wait/drop-consistency complexity. Stop at partial; revisit only if a real-game profile shows `cpu_us` is materially larger.
- **Smoothness/latency vs SteamVR mode** (known limitation — **investigation CLOSED 2026-06-05; do not re-open without a new lever**). Full A/B on RTX 3090 + Quest 3 (same AK app, 72 Hz, H.264, `enforce_server_frame_pacing` on, buffering 2.0). Findings:
  - **Latency is comparable, NOT the cause.** Per-stage via `metrics_export` → ClickHouse: the single-clock-domain stages (encoder/network/decoder/decoder_queue/client_compositor/vsync_queue) are essentially identical across modes. OpenXR's *total_pipeline* reads ~19 ms lower (53.9 vs 72.8 ms) but that's an artifact — `game_time`/`server_compositor`/total cross the client↔server boundary, which has **no clock sync in OpenXR mode**, so they zero out. No real latency gap.
  - **The only real differentiator is the drop rate.** Jitter (steadiest 30 s): SteamVR stddev 1.68 ms / ~3 drops, OpenXR 2.33 ms / ~9 drops per 30 s. The ~3× drop rate is the whole "less smooth / laggier" feel (a hitch reads as both stutter and lag). Drops are **structural** (extra Monado hop + app/`comp_alvr` two-loop bunching).
  - **The phase-sync pacer can't fix it — Slices 1 + 2 both measured-regressed and were reverted.** Slice 0 (the `alvr_oxr_duration_until_next_vsync` bridge getter, ABI v11) landed; Slice 1 (edit `comp_alvr_predict_frame`'s outputs) *regressed*; Slice 2 (sleep-to-cadence at `layer_commit`) was *neutral/worse* (timer over-sleep dropped fps to ~70). Root reason: `duration_until_next_vsync` is a free-running *synthetic* cadence (anchored at connect, never client-corrected), so locking Monado to it doesn't phase-align to the client display.
  - **Verdict: OpenXR mode is in good shape** (comparable latency, modest drop-rate gap); every cheap lever is exhausted. Closing the residual gap needs either a **Slice-3 client-feedback closed-loop** pacer (real work, ~6 fewer hitches/30 s) or **structural** compositor changes (collapse the two-loop / shrink the hop) — **not worth it now; revisit only if drops become a concrete user-blocking complaint.** Full tables, the reverted-slice ledger, and the reusable A/B harness (`cap_jitter*.cmd` on the TESTHOST console + the ClickHouse latency query) are in [`PACING_PHASE_SYNC_SCOPE.md`](PACING_PHASE_SYNC_SCOPE.md) (top "⭐ Measured A/B baseline + decision" block) and `[[openxr-pacing-smoothness]]`.
- **Optional / perf** — Slice 2e (GFX→CS squasher dispatch swap, only if measured); Gate H (per-eye foveation behaviour) on an eye-tracked headset.
- **Small ergonomics** — the original list here is now cleared: auto-build the cdylib (`5dc8d270`), `--monado-source` build flag, and the distinct `EncoderUnavailable` result from `alvr_oxr_submit_layers` (`c618b6a4`) all **landed**. The "populate `comp_scratch_single_images::native_images[idx].size` for the FFR-off path" item was a **misdiagnosis** — upstream Monado already populates it: `vk_ic_allocate` sets the size from `memoryRequirements` (`vk_image_allocator.c:362`) and `comp_scratch.c:243` copies it into `native_images[i].size`, on the same `ensure()` path `chl_scratch` uses for both `_ensure` and `_ensure_mutable`. The real `size == 0` is the *app-swapchain import* path (`vk_ic_from_natives`, `vk_image_allocator.c:493`), which is separate and already worked around by the squasher overwriting `pack` with scratch handles (`e0e48a300`). The `comp_alvr` `submit_no_encoder` counter that acts on the `EncoderUnavailable` code also **landed** (openxr `25b9918cd`, pinned by superproject) — `comp_alvr.c` tallies `submit_layers_no_encoder` apart from generic per-frame failures and reports it on the cumulative `layer_commit diag` line. This sub-list is now fully cleared.
...
| Static FFR right-eye mirror (comp_alvr compress) — right-eye warp fixed (openxr `6aa71133a`, master `4cb8dc39`) | ✅ headset-verified RTX3090+Quest3 | `[[openxr-ffr-right-eye-warp]]` |
| Controllers — completed the `alvr_controller` stub: valid streamed pose + aim (Slice 1, openxr `bc762680a`/master `0e3deb34`) + full Touch input set & `update_inputs` (Slice 2, openxr `2c9abcc57`/master `6e7cf7f8`) | ✅ headset-verified RTX3090+Quest3 | `[[openxr-controllers-stub]]` |
