...
- **Smoothness/latency vs SteamVR mode** (known limitation, measured 2026-06-03) — OpenXR mode feels less smooth / laggier. Both modes run 72Hz (not an fps gap). Measured throughput is fine (72.9fps, encode 4.4ms off-thread, decoder keeps up) but frame-interval **jitter is high** (stddev 2.35ms, min0/max33ms bursting, ~8 likely-drops/30s). Cause: `comp_alvr` paces with Monado's non-phase-aligned `u_pc_fake` helper (vs ALVR's phase-sync pacer in SteamVR mode), plus the extra Monado compositor hop (squash + FFR + residual `vkQueueWaitIdle`) adds baseline latency. Real fix = a phase-aligned pacer in `comp_alvr` that consumes ALVR's existing `duration_until_next_vsync()` (already fed in OpenXR mode; SteamVR mode uses it via `wait_for_vsync`) instead of Monado's free-running `u_pc_fake`. **Scoped into slices in [`PACING_PHASE_SYNC_SCOPE.md`](PACING_PHASE_SYNC_SCOPE.md)** (Slice 0 bridge the cadence → ABI v11; Slices 1–2 enforce it on the comp_alvr producer; Slice 3 close-the-loop + latency budget). Absolute motion-to-photon latency not yet captured (dashboard/ClickHouse only). Measurement method + numbers in `[[openxr-pacing-smoothness]]`.
