...
* Quad / cylinder / equirect / cube / passthrough layer support — rasterise into a single projection image before encoding.
  ...
  * ✅ **Slice 2c.1 LANDED 2026-05-25 (build-clean; smoke verification deferred)** — took path (a): `init_dummy_distortion_target` allocates a throwaway 1x1 `R8G8B8A8_UNORM` `VkImage` + view + `render_gfx_render_pass` + `render_gfx_target_resources` (goto-chain rollback), `compose_via_squasher` attaches it via `chl_frame_state_gfx_set_target` with both views at viewport (0,0,1,1) and identity vertex rots, and `comp_alvr_destroy` tears it down first (reverse-init order). The `#if 0` dispatch wrapper from 2c is now live. The encoder reads the per-view scratch images, not this target, so the distortion output is discarded; cost is a few µs of distortion fragment shader per frame, negligible against the existing `vkQueueWaitIdle` stall. New fields on `struct comp_alvr_compositor` (`dummy_distortion_{image,memory,view,rgrp,rtr}`). Commit ladder: fork `e4b82ed3d` / alvr `156c800f` (submodule bump) / alvr (this doc commit). **Verification ceiling: build-clean only** (`cargo xtask build-openxr-runtime --enable-alvr-driver`, 32s). RTX 3090 smoke is **blocked on an `xrLocateViews` `ERROR_RUNTIME_FAILURE`** that, per the 2026-05-23 bisect, reproduces at the **pre-2c.1 baseline `401d04df8` with all 2c.1 changes reverted** — i.e. it is not caused by this slice. The "30 frames OK" result from Slice 2d's prior session is no longer reproducible on the same host, pointing at a stateful/environmental regression. Tracked separately (see [[next-session-pickup]] for the failure trace + hypotheses); the cheapest next probe is a remote-host reboot, then log lines inside `oxr_session_view.c::oxr_xrLocateViews`.
