...
* Quad / cylinder / equirect / cube / passthrough layer support — rasterise into a single projection image before encoding.
  ...
  * ⏸ Slice 2d — overlay-client smoke. Requires a real OpenXR client driving an overlay layer (hello_xr `--quad-layer`, the Monado `compositor_demo`, or a HUD-using game) against `monado-service.exe`. The bridge layer-type histogram (ABI v3) should report non-zero quad/cylinder/etc. counts; the squasher should run without validation errors; the pre-encoder image should contain the composited overlay (sampled via `u_native_images_debug`, which the scratch `xrt_image_native` integration auto-exposes). End-to-end "the encoded video has the overlay in it" is gated on Slice 3.3 (NVENC body).
