...
* Per-view foveation hint to the OpenXR-mode encoder — **scope nailed down 2026-05-22 in [`PER_VIEW_FOVEATION.md`](PER_VIEW_FOVEATION.md)**. Headline: eye-tracking data (`FaceData.eyes_*`) already arrives in `TrackingData` but isn't piped to the encoder. Per-view variation needs the params to travel per-frame (or low-rate dynamic). Wire-compat win: zero `alvr_packets` break for the v0 case if we route via additive `RealTimeConfig.per_view_foveation: Option<[FoveationView; 2]>`.
  * ✅ **Slice 1 LANDED 2026-05-22** — bridge ABI bumped 4 → 5 with the two new entrypoints: `alvr_oxr_get_foveation(*out_views)` (encoder hot-path read) and `alvr_oxr_set_foveation(*views)` (drain-thread write). New `AlvrOxrFoveationView { is_present, center_size[2], center_shift[2], edge_ratio[2] }` struct mirrors `alvr_session::FoveatedEncodingConfig`. New `static FOVEATION: RwLock<[AlvrOxrFoveationView; 2]>` cache initialises to `{ is_present: false, center_size: 0, center_shift: 0, edge_ratio: 1.0 }` so the encoder defaults to full-resolution before any update lands. Cache lives outside `SERVER_CORE_CONTEXT` on purpose (foveation is config-shaped, survives connect/disconnect cycles, and the encoder hot path shouldn't pay a SERVER_CORE_CONTEXT read just to ask for it). Same single-source-of-truth ABI pattern as hand-tracking v4 (see Bridge ABI section in [[openxr-mode-integration]] memory). Verification: `cargo check -p alvr_server_openxr` clean, `cargo xtask clippy --ci` clean, header regenerated via `ALVR_REGENERATE_BRIDGE_HEADER=1 cargo build -p alvr_server_openxr`. Verification ceiling: bridge surface only; encoder body (Slice 6) is hardware-blocked alongside the existing NVENC body work; producer-side wiring is Slices 2/3/5.
  * ⏸ Slice 2: `alvr_session::FoveatedEncodingConfig` gains an additive `per_view_eye_tracked: Switch<PerViewFoveationConfig>` field (disabled by default). Schema test for the bincode round-trip.
  * ⏸ Slice 3: `server_core` glue — `ServerCoreEvent::PerViewFoveation([FoveationView; 2])` variant + drain-thread translation into `alvr_oxr_set_foveation`.
  * ⏸ Slice 4: `alvr_packets::RealTimeConfig.per_view_foveation: Option<[FoveationView; 2]>` (the one wire-compat coordination point).
  * ⏸ Slice 5: eye-tracking-to-centre math (pure Rust, unit-testable).
