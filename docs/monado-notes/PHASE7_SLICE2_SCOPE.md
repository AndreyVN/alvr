...
### Slice 2a — scope doc (this file)

Establishes the design. ~150 lines. No code, one commit. **THIS slice.**

### Slice 2b — scratch lifecycle
...
### Slice 2c — squasher dispatch in layer_commit

Replace the current `pack_projection_layer` + drop-and-histogram loop with: build a `comp_render_dispatch_data` from `cla->layers`, run `chl_frame_state_init` / `chl_frame_state_finish` around a `comp_render_gfx_dispatch` call, and pack the *scratch* image's native handle into `AlvrOxrLayer.image_handles[v]`. Histogram counts stay (still emitted via `alvr_oxr_report_layer_types`) so the per-window distribution remains observable.

Skip-squash fast path (per L5) kept when there's exactly one projection layer and no overlays — the existing pack code becomes the fast-path branch.
...
### Slice 2d — verify quad/cylinder rasterisation
