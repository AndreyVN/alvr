...
- **Gate B (bridge ABI contract + builder selection)** — Verified 2026-05-21 on the same host. Tip commits: alvr `81426fd4`, openxr submodule `f80c98a7d`. `monado-service.exe` ran without a client connected; log showed `ALVR_INFO [alvr_create_devices] Creating ALVR system devices`, `ALVR driver scaffolding ready (3 devices)`, builder list contained the `alvr: ALVR (streamed)` entry as the selected builder, and `Got devices: 0: ALVR Streamed HMD / 1: ALVR Streamed Controller (L) / 2: ALVR Streamed Controller (R)` with the correct ALVR_HMD / ALVR_Controller_{Left,Right} serials from the bridge. No `ALVR bridge ABI mismatch` line. Service stayed alive waiting for IPC. No `meshuv` compositor warning after fork `f80c98a7d` installed the identity distortion mesh.
