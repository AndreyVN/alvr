...
| `alvr_server_openxr` | cdylib | The OpenXR-mode counterpart to `alvr_server_openvr`. Loaded by the in-fork Monado driver (`openxr/src/xrt/drivers/alvr/`); exports a C ABI (`include/alvr_runtime_bridge.h`, cbindgen-regenerated) covering init/shutdown, head + controller pose, haptics, view params, session events, and (eventually) layer submission. Reuses `alvr_server_core` for the connection state machine. Preview-only; gated behind `Settings.extra.runtime == Openxr` and `cargo xtask build-openxr-runtime --enable-alvr-driver`. |
...
A second PC-side path that replaces SteamVR with a Monado-based OpenXR runtime. Selected per-session via `Settings.extra.runtime` (`Steamvr` default | `Openxr` preview). The two modes share `alvr_server_core` end-to-end; the cdylib that fronts SteamVR is replaced.

1. **Monado driver** (`openxr/src/xrt/drivers/alvr/`, in the fork submodule): the ALVR "fake hardware" driver. Three `xrt_device`s — HMD + left/right controller — implemented in `alvr_hmd.c` and `alvr_controller.c`. `alvr_hub.c::alvr_create_devices` is the entry point Monado's target builder dispatches to. Build-gated by `XRT_BUILD_DRIVER_ALVR=ON`.
2. **Bridge cdylib** (`alvr/server_openxr`, Rust): the driver dlopens it and calls into the C ABI declared in `alvr/server_openxr/include/alvr_runtime_bridge.h`. Bridge ABI is versioned via `ALVR_OXR_BRIDGE_ABI_VERSION` (currently `8`); Monado checks against the cbindgen-emitted `#define` at driver init and refuses to load on mismatch. Key calls: `alvr_oxr_init(root_dir)`, `alvr_oxr_get_head_pose`, `alvr_oxr_get_controller_state`, `alvr_oxr_get_view_params`, `alvr_oxr_set_haptic`, `alvr_oxr_poll_session_event`, `alvr_oxr_submit_layers` (wired to the Vulkan-input NVENC encoder since 2026-05-27), `alvr_oxr_report_pacing(frame_id, begin_ns, submit_begin_ns, submit_end_ns)` (added in ABI v2), `alvr_oxr_report_layer_types(frame_id, n_quad, n_cylinder, n_equirect, n_cube, n_passthrough)` (added in ABI v3 — diagnostic histogram of dropped non-projection layers, Phase 7 Slice 1), plus hand-tracking (`alvr_oxr_get_hand_skeleton`, v4) and per-view foveation (`alvr_oxr_get_foveation` / `alvr_oxr_set_foveation`, v5). Internally the bridge instantiates a `ServerCoreContext`, spawns an `alvr_oxr_event_drain` thread that translates `ServerCoreEvent` variants to `AlvrOxrEvent`s, and caches `LocalViewParams` + controller button state for synchronous reads.
3. **Fake compositor** (`openxr/src/xrt/compositor/alvr/comp_alvr.c`): replaces Monado's `comp_main` when `XRT_FEATURE_COMP_ALVR=ON` is `#define`'d into `xrt_config_build.h` (the cmake_in template must list the flag — easy to forget). `comp_alvr_layer_commit` filters `comp_layer_accum.layers` to `XRT_LAYER_PROJECTION` / `_DEPTH`, packs native image handles + per-view FOV/pose into `AlvrOxrLayer[]`, and calls `alvr_oxr_submit_layers` with the GPU sync handle (Win32 NT / DXGI keyed-mutex / DMABUF fd, cast to `u64`). The same function bookends the bridge call with `u_pc_mark_point(SUBMIT_BEGIN)` / `_END` so Monado's pacer learns the real submission cost, and forwards the captured timestamps through `alvr_oxr_report_pacing` for metrics export (see item 6). `alvr_oxr_submit_layers` is wired (since 2026-05-27) to the Vulkan-input NVENC encoder via the inline `encoder_bridge` module in `server_openxr/src/lib.rs`; verified streaming end-to-end on RTX 3090 + Quest 3.
4. **Encoder reuse** (Phase 3.0 refactor, landed — see `docs/monado-notes/PHASE3_0_SCOPE.md`): the encoder C++ was extracted into a runtime-agnostic `cpp/encoder/{linux,win32_d3d11,win32_vk}/` tree behind a shared `IEncoderBackend` interface (`cpp/encoder/EncoderBackend.h`). On Linux the bridge reuses the existing `cpp/encoder/linux/EncodePipeline*` family (already Vulkan-input). On Windows the `cpp/encoder/win32_vk/VkEncoderBackend` family imports the submitted Vulkan image via `VK_KHR_external_memory_win32`, then encodes through a **CUDA-interop NVENC** path (`VkEncoderBackendC.cpp` + `CudaDriverApi.h`, loading `nvcuda.dll` dynamically) — separate from the D3D11 `cpp/encoder/win32_d3d11/` family OpenVR mode uses. OpenVR's `D3d11EncoderBackend` and OpenXR's `VkEncoderBackend` are the two typed implementations of `IEncoderBackend`.
5. **Runtime registration**: `cargo xtask register-openxr-runtime` makes the built Monado-as-ALVR the active OpenXR runtime for the current user. Windows uses `HKCU\Software\Khronos\OpenXR\1\ActiveRuntime`; Linux uses `$XDG_CONFIG_HOME/openxr/1/active_runtime.json`. Refuses to act while SteamVR's `vrserver` is alive; the inverse direction (`launch_steamvr` refusing when `RuntimeMode::Openxr`) lives in `dashboard/src/steamvr_launcher/mod.rs`. The launcher (`alvr/launcher/`) also surfaces `RuntimeMode` per-installation: each row carries a `[SteamVR (OpenVR)]` / `[Monado (OpenXR) — preview]` badge, and the Edit popup exposes a ComboBox that writes back to `session.json` via `serde_json::Value` manipulation (no schema round-trip).
6. **Pacing telemetry**: `alvr_oxr_report_pacing` (ABI v2) flows into `ServerCoreContext::report_oxr_pacing` → `StatisticsManager::report_oxr_pacing` → `metrics_exporter::Sample::OxrPacing { begin_ns, submit_begin_ns, submit_end_ns }`. The exporter's `Aggregator` derives `cpu_us = submit_begin − begin` and `submit_us = submit_end − submit_begin` per frame, keeps `Acc` distributions across the export window, and emits an `oxr_pacing` JSON section whenever a window saw at least one frame; the field is omitted otherwise so OpenVR-mode snapshots stay byte-identical. Drops silently pre-connection (`metrics_sender == None`).
7. **Layer-type diagnostic** (ABI v3): `alvr_oxr_report_layer_types` flows through the same `ServerCoreContext` → `StatisticsManager` → `metrics_exporter::Sample::OxrLayerTypes` path. The `Aggregator` sums per-type totals (`quad`, `cylinder`, `equirect` — both EQUIRECT1 and EQUIRECT2 share this bucket — `cube`, `passthrough`) across the window and emits an `oxr_layer_types` JSON section with the totals plus a frame counter. Pure diagnostic until Phase 7 Slice 2 lands Vulkan rasterisation; today `comp_alvr` still drops every non-projection layer — this just surfaces what's being dropped so prioritisation can be data-driven.

The migration plan and per-slice progress live at `docs/monado-notes/PHASE3_0_SCOPE.md` and `docs/monado-notes/NEXT_STEPS.md`.
...
- New **bridge ABI surface** (OpenXR mode): bump `ALVR_OXR_BRIDGE_ABI_VERSION` in `alvr/server_openxr/src/lib.rs`, add the Rust extern fn and any new `#[repr(C)]` structs, regenerate the C header (`ALVR_REGENERATE_BRIDGE_HEADER=1 cargo build -p alvr_server_openxr`), and add the Monado-side call site in `openxr/src/xrt/{drivers,compositor}/alvr/`. The Monado-side `ALVR_OXR_BRIDGE_ABI_EXPECTED` check in `alvr_hub.c` uses the cbindgen-emitted `#define`, so the version mismatch surfaces on driver init rather than at runtime. Both halves must land together — either as one ALVR commit that bundles a fork commit (via submodule bump) plus the Rust+header diff, or as two consecutive commits with the second being the bump. Never push the Rust half without the matching fork commit available, or every dev hosting the openxr submodule at the previous tip will hit ABI mismatch.
