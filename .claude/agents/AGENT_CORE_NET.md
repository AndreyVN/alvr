...
| `alvr/session` | `alvr_session` | Settings schema (derived via `settings-schema-rs`, pinned git rev), `SessionConfig`/`Settings`, migrations between versions. Note: the user-facing "schema" lives here — there is no separate `alvr/schema` crate in this fork. The top-level `Settings.metrics: MetricsConfig` field (sibling of `video`/`audio`/`headset`/`connection`/`extra`) carries the streaming-metrics + hwmonitor export config and the `extended_headset_telemetry` toggle — keep its serde shape stable across releases and add migrations for any rename. |
| `alvr/packets` | `alvr_packets` | Every wire type exchanged between streamer and client: `ClientControlPacket`, `ServerControlPacket`, `TrackingData`, `VideoPacketHeader`, `StreamConfigPacket`, `BatteryInfo`, `ClientTelemetry`, etc. The numeric stream IDs (`TRACKING`, `VIDEO`, `AUDIO`, `HAPTICS`, `STATISTICS`) are also here. Note: controller battery rides the existing `ClientControlPacket::Battery(BatteryInfo)` variant — the recipient discriminates HMD vs. left/right by `device_id` (`HEAD_ID` / `HAND_LEFT_ID` / `HAND_RIGHT_ID`). The newer headset health payload lives in `ClientControlPacket::Telemetry(ClientTelemetry)`; every field is `Option<_>` to allow per-sensor omission, so add new ones the same way rather than introducing parallel variants. |
