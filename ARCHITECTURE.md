...
2. **Bridge cdylib** (`alvr/server_openxr`, Rust): the driver dlopens it and calls into the C ABI declared in `alvr/server_openxr/include/alvr_runtime_bridge.h`. Key calls: `alvr_oxr_init(root_dir)`, `alvr_oxr_get_head_pose`, `alvr_oxr_get_controller_state`, `alvr_oxr_get_view_params`, `alvr_oxr_set_haptic`, `alvr_oxr_poll_session_event`, `alvr_oxr_submit_layers` (stubbed pending the Vulkan-input encoder). Internally the bridge instantiates a `ServerCoreContext`, spawns an `alvr_oxr_event_drain` thread that translates `ServerCoreEvent` variants to `AlvrOxrEvent`s, and caches `LocalViewParams` + controller button state for synchronous reads.
3. **Fake compositor** (`openxr/src/xrt/compositor/alvr/comp_alvr.c`): replaces Monado's `comp_main` when `XRT_FEATURE_COMP_ALVR=ON`. Picks up submitted OpenXR layers, packages native image handles into `AlvrOxrLayer[]`, and calls `alvr_oxr_submit_layers`. Current state: stub returning `XRT_ERROR_NOT_IMPLEMENTED`; the real body is the open work.
