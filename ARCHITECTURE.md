...
2. **Bridge cdylib** (`alvr/server_openxr`, Rust): the driver dlopens it and calls into the C ABI declared in `alvr/server_openxr/include/alvr_runtime_bridge.h`. Bridge ABI is versioned via `ALVR_OXR_BRIDGE_ABI_VERSION` (currently `2`); Monado checks against the cbindgen-emitted `#define` at driver init and refuses to load on mismatch. Key calls: `alvr_oxr_init(root_dir)`, `alvr_oxr_get_head_pose`, `alvr_oxr_get_controller_state`, `alvr_oxr_get_view_params`, `alvr_oxr_set_haptic`, `alvr_oxr_poll_session_event`, `alvr_oxr_submit_layers` (stubbed pending the Vulkan-input encoder), and `alvr_oxr_report_pacing(frame_id, begin_ns, submit_begin_ns, submit_end_ns)` (added in ABI v2). Internally the bridge instantiates a `ServerCoreContext`, spawns an `alvr_oxr_event_drain` thread that translates `ServerCoreEvent` variants to `AlvrOxrEvent`s, and caches `LocalViewParams` + controller button state for synchronous reads.
