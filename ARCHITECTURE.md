...
2. **Bridge cdylib** (`alvr/server_openxr`, Rust): the driver dlopens it and calls into the C ABI declared in `alvr/server_openxr/include/alvr_runtime_bridge.h`. Bridge ABI is versioned via `ALVR_OXR_BRIDGE_ABI_VERSION` (currently `3`); Monado checks against the cbindgen-emitted `#define` at driver init and refuses to load on mismatch. Key calls: `alvr_oxr_init(root_dir)`, `alvr_oxr_get_head_pose`, `alvr_oxr_get_controller_state`, `alvr_oxr_get_view_params`, `alvr_oxr_set_haptic`, `alvr_oxr_poll_session_event`, `alvr_oxr_submit_layers` (stubbed pending the Vulkan-input encoder), `alvr_oxr_report_pacing(frame_id, begin_ns, submit_begin_ns, submit_end_ns)` (added in ABI v2), and `alvr_oxr_report_layer_types(frame_id, n_quad, n_cylinder, n_equirect, n_cube, n_passthrough)` (added in ABI v3 — diagnostic histogram of dropped non-projection layers, Phase 7 Slice 1). Internally the bridge instantiates a `ServerCoreContext`, spawns an `alvr_oxr_event_drain` thread that translates `ServerCoreEvent` variants to `AlvrOxrEvent`s, and caches `LocalViewParams` + controller button state for synchronous reads.
3. **Fake compositor** (`openxr/src/xrt/compositor/alvr/comp_alvr.c`): replaces Monado's `comp_main` when `XRT_FEATURE_COMP_ALVR=ON` is `#define`'d into `xrt_config_build.h` (the cmake_in template must list the flag — easy to forget). `comp_alvr_layer_commit` filters `comp_layer_accum.layers` to `XRT_LAYER_PROJECTION` / `_DEPTH`, packs native image handles + per-view FOV/pose into `AlvrOxrLayer[]`, and calls `alvr_oxr_submit_layers` with the GPU sync handle (Win32 NT / DXGI keyed-mutex / DMABUF fd, cast to `u64`). The same function bookends the bridge call with `u_pc_mark_point(SUBMIT_BEGIN)` / `_END` so Monado's pacer learns the real submission cost, and forwards the captured timestamps through `alvr_oxr_report_pacing` for metrics export (see item 6). The Vulkan-input NVENC body that `alvr_oxr_submit_layers` will eventually call is the remaining open work; today the call returns `NotImplemented` and `layer_commit` demotes that to DEBUG so the per-frame log stays quiet.
4. **Encoder reuse**: on Linux the bridge will reuse the existing `cpp/encoder/linux/EncodePipeline*` family (already Vulkan-input, runtime-agnostic). On Windows a separate `cpp/encoder/win32_vk/VkEncoderBackend` family handles Vulkan-image input via `VK_KHR_external_memory_win32` + NVENC's `nvEncRegisterResource` Vulkan-image API (separate from the existing D3D11 `cpp/encoder/win32_d3d11/` family that OpenVR mode uses). Both implement the shared `IEncoderBackend` interface in `cpp/encoder/EncoderBackend.h`.
